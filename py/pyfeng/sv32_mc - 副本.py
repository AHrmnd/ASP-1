...
class Sv32McAe2(sv32_mc2.Sv32McABC):
    ...
    def generate_VT(self, X_T): 
     ...
     #   X_T = generate_XT()
     #   V_T = generate_VT(X_T)
     #   SPOT = np.log(X_T/X_0)+(self.mr+self.vov**2/2)*V_T-self.mr*self.theta*self.texp)*self.rho/self.vov - 0.5*V_T
     #   SIG = np.sqrt((1.0 - self.rho**2) * V_T / self.texp
     #   return Bsm(strike, SPOT, SIG) 
    ...
    def get_price(self, strike=1, spot=1, texp=1, cp=1):
        X_0 = 1 / self.sigma ** 2
        prices = self.price(strike, spot, texp, cp)
        return prices
...
class Sv32McAe(abc.SvABC):
    def __init__(
        ...
        #self.bsm_model = pf.Bsm(sigma = self.sigma, intr=intr, divr=divr)
        #self.norm_model = pf.Norm(sigma = self.sigma, intr=intr, divr=divr)
    ...
    def calOutput(self):
        ...
        """
        if self.beta == 0:
            prices = self.norm_model(strike, spot, sigma, texp, cp, r)
                #self.E_F_T, self.Ks, self.r, self.T, self.sigma_N, self.cp)
        else:
            prices = self.bsm_model.price(strike, spot, sigma, texp, cp, r)
    ...
    def calForwardAndVolatility_version2(self, M1):
        ...
        if self.beta == 0:
            ...
            self.sigma_N = np.sqrt((1 - self.rho ** 2) * M1 / self.texp)
            self.sigma_N = self.sigma_N.reshape(-1, 1)
        else:
            outside_term = self.rho ** 2 * M1 / 2
            self.E_F_T = self.S0 * np.exp(internal_term - outside_term)
            self.sigma_BS = np.sqrt((1 - self.rho ** 2) * M1 / self.texp).reshape(-1, 1)
        self.E_F_T = self.E_F_T.reshape(-1, 1)
    ...
    def simulate_Heston_outlayer(self):
        ...
        c_T = self.c_function(self.texp)
        alpha = X_0 / c_T
        exp_term = np.exp(self.kappa * self.theta * self.texp)
        return (
            np.random.noncentral_chisquare(delta, alpha, size=self.path_num)
            / exp_term
            * c_T
    ...
    def c_function(self, t):
        ...
        Args:
            t: float t is the independent variable of c function
        Returns:
            float
                c(t)
        """
