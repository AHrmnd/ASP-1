...
class Sv32McAe2(sv32_mc2.Sv32McABC):
    """
    Almost Exact MC for 3/2 model

    Example:
        >>> import numpy as np
        >>> import pyfeng as pf
        >>> sv32_mc_ae = pf.Sv32McAe2(spot=1, strike=1, intr=0.05, texp=1, sigma=1, beta=1, rho=-0.5, theta=1.5, mr=2, vov=0.2, path_num = 1000, cp=1)
        >>> output1 = sv32_mc_ae.get_price(strike=1, spot=1, texp=1, cp=1)
